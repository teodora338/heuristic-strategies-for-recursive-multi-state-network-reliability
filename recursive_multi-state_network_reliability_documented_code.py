...
def test(true, recursive):
    """Accuracy check: testing equality between the true probability and the probability returned by the recursive function"""

    true = true[-7:]
    recursive = recursive[-7:]
...
def results(vectors, probs, true):
    """Calling the iterative and recursive probability functions, printing the results, and performing an accuracy check."""

    mpv_tuples = [tuple(abs(x) for x in v) for v in vectors]

    iterative = iterative_reliability(mpv_tuples, probs)
    recursion = recursive_reliability(method, metric, centroid_method, algorithm, mpv_tuples, probs)

    output_true = f"Correct Result = {true:.5f}"
    output_iterative = f"Iterative Result = {iterative:.5f}"
    output_rec = f"Recursive Result = {recursion:.5f}"

    print(output_true)
    print(output_iterative)
    print(output_rec)
    test(output_true, output_rec)

    return iterative, recursion
...
if __name__ == "__main__":
    ...
    component_probs1 = [p1, p2, p3, p4, p5]
    correct_result1 = 0.97686
    it1, rec1 = results(mpv1, component_probs1, correct_result1)
    ...
    correct_result2 =0.85041
    it2, rec2 = results(mpv2, component_probs2, correct_result2)
    reliability_cache.clear()
    ...
    correct_result3 = 0.84614
    it3, rec3 = results(mpv3, component_probs3, correct_result3)
    reliability_cache.clear()
